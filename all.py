...
def evaluate_agent(agent, env, num_episodes=5):
    ...
    for i in range(num_episodes):
        ...
        step = 0
        print(f"
Episode {i+1} — Goal: {inner_env.goal_word}")
        ...
        while not done:
            ...
            with t.no_grad():
                ...
                if isinstance(obs, dict) and "guessed" in obs:
                    guessed = t.as_tensor(obs["guessed"], device=logits.device).bool()
                    logits[0, guessed] = -1e9
                action = t.argmax(logits, dim=-1).item()

            obs, reward, terminated, truncated, _ = env.step(action)
            done = terminated or truncated
            last_guess, last_mask = inner_env.history[-1]
            print(f"  Guess {step+1}: {last_guess}  {last_mask}")
            step += 1

        print("  Win!" if terminated else "  Loss.")
        successes += int(terminated)
        total_guesses += step

    print(f"
Win rate: {successes}/{num_episodes} ({100 * successes / num_episodes:.1f}%)")
    print(f"Avg guesses: {total_guesses / num_episodes:.2f}")
...
def generate_expert_dataset(env, preprocess_obs, opening_book, num_episodes: int = 100):
    ...
    states = t.vstack(states)
    actions = t.tensor(actions, dtype=torch.long)
    ent_drops = t.tensor(ent_drops, dtype=torch.float32)
    return states, actions, ent_drops
    

def behavioral_clone(actor, decoder, states, actions, epochs=5, batch_size=128, lr=1e-3, device=device):
    """Warm-start actor+decoder via supervised imitation of expert actions."""
    optimizer = optim.Adam(list(actor.parameters()) + list(decoder.parameters()), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        for batch_states, batch_actions in loader:
            ...
            state_emb = actor(batch_states)
            logits = decoder(state_emb)
...
bc_states, bc_actions, _ = generate_expert_dataset(bc_env, preprocess_obs, opening_book_full, num_episodes=500)
behavioral_clone(
...
for i, stage in enumerate(curriculum):
    ...
    t.save(master_agent.state_dict(), ckpt_path)
    print(f"Saved model checkpoint to {ckpt_path}")

    eval_env = gym.make("Wordle-v0-Full")
    evaluate_agent(master_agent, eval_env, num_episodes=100)
